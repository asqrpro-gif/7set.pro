...
- seo_title: Идеальный SEO-заголовок (40-75 символов). Обязательно включи код, марку, модель. ВМЕСТО шаблонных фраз типа "причины и ремонт", обязательно укажи название неисправного узла/датчика из фактов (например: "Ошибка P0172 Chery Tiggo 4: проблемы с ДМРВ и лямбда-зондом").
- seo_description: Идеальное SEO-описание (110-155 символов). Укажи суть ошибки на ${displayBrand} ${displayModel} и 1-2 конкретных узла, которые вызывают проблему. Без воды и шаблонных вопросов ("Загорелась ошибка?"). Пиши плотно.`;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (googleResponse.ok && responseText.trim().startsWith('{')) {
          ...
          const textContent = data.candidates?.[0]?.content?.parts?.[0]?.text;
          if (textContent) {
              seoData = JSON.parse(textContent);
        ...
        if (attempt === maxAttempts) console.error(`⚠️ [AI DEEP ERROR] Этап 2 (SEO) не удался: ${error.message}`);
      }
    }
  }
  ...
  return {
    ...
    seo_title: `Ошибка ${displayCode} ${displayBrand} ${displayModel}: причины и диагностика`,
    seo_description: `Расшифровка ошибки ${displayCode} на ${displayBrand} ${displayModel}. Основные симптомы, причины возникновения и как устранить неисправность своими руками или на СТО.`,
    faq_items: [
