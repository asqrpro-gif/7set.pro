...
is_unsupported_error: всегда false.`;
  ...
  try {
    ...
    if (googleResponse.ok && responseText.trim().startsWith('{')) {
      ...
      console.error(`⚠️ [AI FAST ERROR] Прокси вернул ошибку или HTML. Ответ: ${responseText.substring(0, 100)}`);
    }
  } catch (error) {
    console.error(`⚠️ [AI FAST ERROR] Таймаут или сбой: ${error.message}`);
  }
