...
const UNSUPPORTED_ERROR_TEMPLATE = {
  ...
  seoDescription: "Код ошибки не найден в каталогах. Узнайте, почему сканеры выдают неизвестные коды и как проверить ЭБУ без лишних затрат.",
...
ПОЛЯ:
- seoTitle: Строгий формат: "Ошибка ${displayCode} ${displayBrand} ${displayModel}: [Суть проблемы]".
  СТРОГИЙ ЗАПРЕТ: Никогда не используй слова "расшифровка", "причины" или "что значит" в seoTitle.
  Запрещен шаблон: "Ошибка [Код] [Авто]: расшифровка и причины".
  Требование: Заголовки должны быть разнообразными, кликабельными (CTR) и отражать суть проблемы или последствия (например: "Ошибка ${displayCode} ${displayBrand}: можно ли ехать дальше?", "Сбой ${displayCode} на ${displayModel} — цена ремонта", "Критическая ошибка ${displayCode} ${displayBrand}: как избежать поломки двигателя").
...
Остальные поля (цены, сложность, инструменты) заполни реалистично.`;
