...
export async function calculateSeoScore(report, prisma) {
  ...
  if (!isStub) {
    score += 15;
    details.push({ label: "Заглушки ИИ", score: 15, max: 15, status: "success", text: "Шаблонные фразы ИИ не обнаружены." });
  } else {
    details.push({ label: "Заглушки ИИ", score: 0, max: 15, status: "danger", text: "Обнаружены стоп-фразы («сбой по коду», «указывает на проблему...»). Текст выглядит шаблонно." });
  }
  ...
  if (prisma) {
    try {
      ...
      if (exactMetaDuplicate) {
        ...
        // Начисляем баллы пропорционально уникальности (максимум 25)
        uPoints = Math.round((uniquenessScore / 100) * 25);
        score += uPoints;

        if (uniquenessScore < 25) {
          details.push({ label: "Уникальность текста", score: uPoints, max: 25, status: "danger", text: `КРИТИЧНО! Уникальность всего ${uniquenessScore}%. Текст слишком похож на другие карточки.` });
        } else if (uniquenessScore < 70) {
          details.push({ label: "Уникальность текста", score: uPoints, max: 25, status: "warning", text: `Уникальность ${uniquenessScore}%. Желательно переписать, чтобы повысить оценку.` });
        } else {
  ...
  let risk = 'DANGER';
  // Принудительно DANGER если уникальность < 25%
  if (uniquenessScore < 25) {
    risk = 'DANGER';
  } else if (score >= 80) {
    risk = 'SAFE';
