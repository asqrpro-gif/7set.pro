...
export function enrichReportText(text, brand, model, code, drivability) {
    ...
    const footer = `\n\n### Полезные ссылки\n\n<a href="${obdUrl}" target="_blank" rel="nofollow noopener" class="inline-flex items-center gap-2 bg-slate-50 dark:bg-slate-700/50 text-blue-600 dark:text-blue-400 px-4 py-3 rounded-xl font-medium transition-colors hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-600 w-full mt-2"><i data-lucide="external-link" class="w-5 h-5"></i> ${obdTitle}</a>`;
