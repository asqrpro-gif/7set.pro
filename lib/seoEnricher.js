...
export function enrichReportText(text, brand, model, code, drivability) {
    ...
    for (const item of wikiDictionary) {
        let replaced = false;
        let firstMatch = '';
        text = text.replace(item.regex, (match, offset, string) => {
            if (!replaced) {
    ...
    if (dictionaryTerms.size > 0) {
        const dictSection = `\n\n<!-- GLOSSARY_START -->
<div class="seo-glossary-card">
  <div class="seo-glossary-header">
    <div class="seo-glossary-icon">
      <i data-lucide="book-open"></i>
    </div>
    <div class="seo-glossary-title">Глоссарий терминов</div>
  </div>
  <div class="seo-glossary-tags">
${Array.from(dictionaryTerms).map(t => `    <span class="seo-glossary-tag">${t}</span>`).join('\n')}
  </div>
</div>
<!-- GLOSSARY_END -->\n\n`;
